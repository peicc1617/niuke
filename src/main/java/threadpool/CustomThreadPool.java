...
//            LOGGER.info("线程池已经关闭，不能再提交任务！");
            System.out.println("线程池已经关闭，不能在提交任务");
            return;
        }
...
//                LOGGER.error("超过最大线程数");
                ...
                try {
                    //会阻塞
                    workQueue.put(runnable);//put与offer的区别
    ...
    private void addWorker(Runnable runnable) {
        Worker worker = new Worker(runnable, true);//新创建一个工作线程
        worker.startTask();//内部调用线程的run()方法运行线程，执行任务
        workers.add(worker);//线程存放到线程池中
    }
    ...
    /*private*/ final class Worker extends Thread {
        ...
        public void run() {
            ...
            try {
                while ((task != null || (task = getTask()) != null)) {//如果不是新任务，则从任务队列queue中获取任务
                    try {
                        //执行任务
                        task.run();
                    } finally {
...
//                        LOGGER.info("number={}",number);
                        System.out.println("当前任务数量："+number);
                        if (number == 0) {
                            synchronized (shutDownNotify) {
                                System.out.println("没有任务需要完成时通知其他线程（也就是主线程，此处指的是StartCustomThreadPool）");
                ...
                System.out.println("移除线程："+remove+workers.size());
                tryClose(true);
            }
        }
    }
